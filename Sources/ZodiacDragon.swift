...
extension PelicanArt {
    static func drawDragonBody(_ c: inout GraphicsContext, t: Double, d: PelicanDraw,
        ...
        for sx in [-20.0, -8.0, 4.0] {
            ...
            s.addLine(to: CGPoint(x: sx + 5, y: -30))
        ...
        hornL.addQuadCurve(to: CGPoint(x: -16, y: -24), control: CGPoint(x: -17, y: -15))
        hg.stroke(hornL, with: .color(goldC), style: StrokeStyle(lineWidth: 4, lineCap: .round))
        var hornR = Path()
        hornR.move(to: CGPoint(x: 4, y: -14))
        hornR.addQuadCurve(to: CGPoint(x: 2, y: -26), control: CGPoint(x: 8, y: -24))
        ...
        // 金色龙须（下垂）
        var barbelL = Path()
        barbelL.move(to: CGPoint(x: 10, y: 0))
        barbelL.addQuadCurve(to: CGPoint(x: 14, y: 12), control: CGPoint(x: 16, y: 4))
        hg.stroke(barbelL, with: .color(goldC), style: StrokeStyle(lineWidth: 2, lineCap: .round))
        var barbelR = Path()
        barbelR.move(to: CGPoint(x: 16, y: 2))
        barbelR.addQuadCurve(to: CGPoint(x: 22, y: 13), control: CGPoint(x: 22, y: 5))
        hg.stroke(barbelR, with: .color(goldC), style: StrokeStyle(lineWidth: 2, lineCap: .round))
        // 粉鼻 + 嘴
        hg.fill(Path(ellipseIn: CGRect(x: 16, y: -5, width: 6, height: 5)), with: .color(nosePink))
        var mouth = Path()
        mouth.move(to: CGPoint(x: 19, y: 1))
        mouth.addQuadCurve(to: CGPoint(x: 13, y: 5), control: CGPoint(x: 18, y: 4))
        hg.stroke(mouth, with: .color(.black.opacity(0.4)), style: StrokeStyle(lineWidth: 1.3, lineCap: .round))
        if p.mouthOpen {
            hg.fill(Path(ellipseIn: CGRect(x: 14, y: 1.5, width: 7, height: 5)), with: .color(.black.opacity(0.3)))
        }
        ...
        return HeadInfo(center: CGPoint(x: 30, y: -27), radius: 16, mouthTip: CGPoint(x: 49, y: -29))
    }
}
